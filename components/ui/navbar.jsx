      ...
      <motion.div
        ...
        {navItems.map((navItem, idx) => (
          <Link
            ...
            href={navItem.link}
            className={cn(
              "relative group dark:text-neutral-50 items-center flex space-x-1 text-neutral-600 dark:hover:text-neutral-300 hover:text-neutral-500"
            )}>
            <span className="block sm:hidden">{navItem.icon}</span>
            <span className="hidden sm:block text-sm">{navItem.name}</span>
            <span className="absolute inset-x-0 w-full mx-auto -bottom-px group-hover:bg-gradient-to-r from-transparent via-blue-500 to-transparent  h-px" />
          </Link>
        ))}
      </motion.div>
    </AnimatePresence>
